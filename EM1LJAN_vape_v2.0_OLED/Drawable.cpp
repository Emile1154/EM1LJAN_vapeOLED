...
void Drawable::initDisp(){
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // Address 0x3D for 128x64
    Serial.println(F("SSD1306 error, check connection or change adress"));
...
void Drawable::drawBattery(){
  ...
  if(analogRead(viewChardge) < 900){     //если на входе зарядки меньше 4,5 В
    //careful = false;                    // разрешить парить
    ...
    //careful = true;                 //запрет на парение
