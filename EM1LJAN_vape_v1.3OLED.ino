/*
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    Переделанная прошивка GyverMOD v1.3 под OLED дисплей 128x32 px  
  исходник: GyverMOD https://github.com/AlexGyver/GyverMOD
    ВНИМАНИЕ! ПРОШИВКА ПОД ВЕРТИКАЛЬНУЮ ОРИЕНТАЦИЮ ДИСПЛЕЯ
    При первом запуске initial_calibration должен быть равен 1 (строка №38)
  ...
  для тех, кто хочет поставить свою анимацию:  
    ...
    -массивы one, three, four - кадры при включении, второй кадр не влез, многа жрет дисп пзу( 
...
//-------------МАССИВЫ ИЗОБРАЖЕНИЙ------------------
static unsigned char one[] U8G_PROGMEM = {
0x00,0x00,0x00,0x00,
0x00,0xF8,0x07,0x00,
0x00,0x0F,0x78,0x00,
0x80,0x02,0x80,0x01,
...
static unsigned char fps2[] U8G_PROGMEM = {
  0x00,0x00,0x00,0x00,
...
void setup(void) {
  ...
  if (bat_vol < battery_low * 1000) {
    ...
    lowBattery();                 // отобразить напряжение
  }else{
    flag = 1;
  }
}
...
void drawRocket(){
  if(flagR){
    ...
    u8g.drawXBMP( 0, 0, 32, 128, fps2);  //рисуем 2 кадр
  }
}
//----------------анимация при парении---------------------
//-------------------главный метод-------------------------
...
void lowBattery(){
  u8g.firstPage();      
  do{
    drawBattery();           // показывать только батарею
    if(readVcc() > battery_low*1000){
      flag = true;
      break;
    }
  }while(u8g.nextPage());  
...
void draw(byte b) {
    if(b == 0 || b ==4){
     u8g.drawXBMP( 0, 50, u8g_logo_width, u8g_logo_height, one);
    }
    else if (b == 1 || b == 3){
